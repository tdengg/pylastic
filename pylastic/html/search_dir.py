...
class SearchDir(object):
    def __init__(self, files=['vasprun.xml'], root='./', xmlout=False):
        self.files = files
        self.root = root
        os.chdir(self.root)
        self.xmlout = xmlout
            ...
            for dir in currDir:
                if os.path.isdir(dir):
                    ...
                    while i < len(self.files):
                        if os.path.exists(self.files[i]) and os.path.exists("vasprun.xml"):
                    ...
                    if os.path.exists("vasprun.xml") and self.xmlout == True:                       #1                        
                        ...
                        dirx[self.n].setAttribute("thermo", str(thermodata))
